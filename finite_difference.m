...
% Square wave
% f(x>-0.4&x<0.4) = 1;
f(x>-0.0025&x<0.0025) = 1;

% Sine wave
% f = sin( 15*x/(x_max-x_min)*2*pi );
...
%     z  = z - Diff6(z,nx);
    ...
    disp([' dtau_n = ',num2str((tau_n-dt)/dt,'%5.5e'),...
...
% 1st upwind
function DF = upwind1(F,dx,nx,u)
if u(1)>=0
    DF(2:nx,1) = (-F(1:nx-1) + F(2:nx))/dx;
    DF(1     ) = (-F(nx    ) + F(1   ))/dx;
else
    DF(1:nx-1,1) = (-F(1:nx-1) + F(2:nx))/dx;
    DF(nx      ) = (-F(nx    ) + F(1   ))/dx;
end
end
